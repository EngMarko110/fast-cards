...
@Component({ selector: 'admin-sub-categories-form', templateUrl: './sub-categories-form.component.html', styles: [] })
export class SubCategoriesFormComponent implements OnInit, OnDestroy {
  ...
  public onSubmit(): void {
    ...
    const subCategory: SubCategory = { _id: this.currentSubCategoryId, parentCategory: this.currentCategoryId, name: this.subCategoryForm.name.value };
  ...
  private _addSubCategory(subCategory: SubCategory): void {
    this.categoriesService.createSubCategory(subCategory).pipe(takeUntil(this.endsubs$)).subscribe((subCategory: SubCategory) => {
      this.messageService.add({ severity: 'success', summary: 'Success', detail: `Sub Category ${subCategory.name} is created!` });
  ...
  private _updateSubCategory(subCategory: SubCategory): void {
    this.categoriesService.updateSubCategory(subCategory).pipe(takeUntil(this.endsubs$)).subscribe(() => {
    ...
    this.route.params.pipe(takeUntil(this.endsubs$)).subscribe((params) => {
      if (params.parentCategory) {
        ...
        if (params.id) {
          ...
          this.categoriesService.getSubCategory(params.id).pipe(takeUntil(this.endsubs$)).subscribe((subCategory) => {
  ...
  get subCategoryForm() { return this.form.controls; }
}
